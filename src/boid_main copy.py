...
class MiRoClient:
    ...
    # SAFE_DISTANCE = 0.1392667442560196
    ...
    def lock_onto_miro(self, error=25):
        ...
        if self.just_switched:  # Print once
            print("MiRo is locking on to the target...")
            self.just_switched = False
            self.lock_start_time = rospy.Time.now().to_sec()
        ...
        # Timeout: give up and kick only if still trying to lock
        if self.status_code == 2 and rospy.Time.now().to_sec() - self.lock_start_time > self.max_lock_duration:
            rospy.loginfo("[LOCK TIMEOUT] Too long aligning, proceeding to kick.")
            self.status_code = 3
            self.just_switched = True
    ...
    def kick(self):
        ...
        if self.just_switched:
            ...
            self.just_switched = False
            self.kick_end_time = rospy.Time.now().to_sec() + 1.0  # 1 second from now
            return

        # Check for face detection
        for index in range(2):  # Check both cameras
            if self.new_frame[index]:
                image = self.input_camera[index]
                if self.detect_face_strict(image):
                    rospy.loginfo("[FACE DETECTED] Turning 180° instead of kicking.")
                    start_time = rospy.Time.now().to_sec()
                    while rospy.Time.now().to_sec() - start_time < self.TURN_DURATION and not rospy.core.is_shutdown():
                        self.drive(speed_l=self.TURNING_FACTOR, speed_r=-self.TURNING_FACTOR)
                        rospy.sleep(self.TICK)
                    self.status_code = 0  # Back to search or default
                    self.just_switched = True
                    return

        # Proceed with kicking if no face is detected
        if rospy.Time.now().to_sec() < self.kick_end_time:
            if self.sonar_distance is not None and self.sonar_distance > self.SAFE_DISTANCE:
                self.drive(self.BASE_SPEED, self.BASE_SPEED)
                self.kick_stop_counter = 0  # ← Reset here on successful kick forward
            else:
                rospy.loginfo("[KICK STOP] Obstacle too close during kick! Halting.")
                self.drive(0.0, 0.0)
                self.kick_stop_counter += 1

                if self.kick_stop_counter >= self.kick_stop_limit:
                    rospy.logwarn("[ESCAPE] Too many KICK STOPs, turning 180° to reset.")
                    start_time = rospy.Time.now().to_sec()
                    while rospy.Time.now().to_sec() - start_time < self.TURN_DURATION and not rospy.core.is_shutdown():
                        self.drive(speed_l=self.TURNING_FACTOR, speed_r=-self.TURNING_FACTOR)
                        rospy.sleep(self.TICK)
                    self.status_code = 0
                    self.just_switched = True
                    self.kick_stop_counter = 0  # Reset counter
